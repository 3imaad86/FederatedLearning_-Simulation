...
def train(msg: Message, context: Context):
    ...
    # F_k = loss du modele global sur la partition locale, AVANT training
    # (signal de "satisfaction"/disparite du client)
    ...
    # entrainement standard. La passe F_k fait partie du cout FairFed.
    train_loss, _ = train_fn(model, trainloader, epochs, lr, device,
                             momentum=c["momentum"])
    local_time_s = time.perf_counter() - t0

    # evaluation locale + envoi de f_k
    ...
    return make_train_reply(
        ...
        extra_metrics=extra,
        # F_k forward-only -> ~0.5 epoch compute extra
